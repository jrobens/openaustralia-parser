...
class MEMBER {
	...
	function MEMBER ($args) {
		...
		// Get the data.
// HACK - JR _ remove person_id
		$q = $this->db->query("SELECT mem.member_id, mem.house, mem.title, mem.first_name, mem.last_name, 
			mem.constituency, mem.party, mem.entered_house, mem.left_house, mem.entered_reason, 
			mem.left_reason, mem.member_id, info.data_value as email 
		FROM member mem INNER JOIN memberinfo info ON mem.member_id = info.member_id
		WHERE mem.member_id = '" . mysql_escape_string($person_id) . "'  AND info.data_key = 'email'
		...
		for ($row=0; $row<$q->rows(); $row++) {
			...
			if ( $house==0 					# The Monarch
				...
				$this->email		= $q->field($row, 'email');
				$this->person_id	= $q->field($row, 'member_id');
			}
	...
	function member_id_to_person_id ($member_id) {
		global $PAGE;
		// JR hack
		$q = $this->db->query("SELECT member_id FROM member 
					WHERE member_id = '" . mysql_escape_string($member_id) . "'");
		if ($q->rows > 0) {
			return $q->field(0, 'member_id');
		} else {
...
// JR Hack
		...
		if ($q->rows > 0) {
			...
			$q = $this->db->query("SELECT member_id FROM member WHERE constituency = '".mysql_escape_string($constituency)."' ORDER BY left_house DESC LIMIT 1");
			if ($q->rows > 0) {
				return $q->field(0, 'member_id');
			} else {
